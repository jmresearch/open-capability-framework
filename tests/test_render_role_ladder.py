...
def test_renders_bar_verbatim_plus_evidence(fixture_role):
    ...
    assert em14[5] == f"Depth: {catalog_bar('EM-14', 'P2')}"  # no evidence clause
...
def test_renders_none_evidence_as_bar_only(fixture_role):
    from render_role_ladder import render_role
    path = os.path.join(ROOT, "roles", fixture_role, "role.yaml")
    role = yaml.safe_load(open(path))
